### Import
# Pour la gestion des date & heures
import time
import datetime as dt

# Modules relatifs au capteurs
...
# Module pour la gestion de la camera 
from picamera import PiCamera

# Module personnel - gestion de tableau de données
from LastDataDf import LastDataDf

## Intégration de données
# Systeme de queuing : MQTT + mousquitto
...
class Gateway:
	def __init__(self):
		...
		self.liste_entete =["time", "temperature", "humidity", "noise", "isPeopleDetected", "TVoC", "CO2eq", "alerte_aeration", "alerte_intrusion", "alerte_incendie", "image"]
		...
		self.delta_time_writing  = 60 # temps en secondes
		...
		# Initialisation du client MQTT pour l'envoi de données
		# client_id = "3" 	# il s'agit du numéro de la boite de TP (numero du Raspberry utilisé)
		self.client_id = 3		
		self.client = paho.Client()
		# connection à l'IP WLAN du raspberry PI qui va host les différents Topic MQTT
		self.client.connect('192.168.220.62', 1883)
	...
	def graph(self):
		# scenario + traitement
		print("Graph d'état")
		print(self.b1_status)
		print(self.etat)
		print(self.current_datetime.isoweekday())
		print(self.current_datetime.time())
		print(self.alerte_etat_1)
		print(self.alerte_etat_2)
	...
	def outputUpdate(self):
		...
		##### Ecriture/Envoi des données - ici toutes les 3 minutes #####
		...
		if self.last_time_writing == None or self.current_datetime >= self.last_time_writing + dt.timedelta(seconds= self.delta_time_writing) :
			...
			### Formatage des données pour l'écriture
			self.liste_input = []
			# convertir la valeur de time() renvoyée par python
			self.liste_input.append(str(dt.datetime.now()))
			...
			for value in dico_writing_values.values() :
				self.liste_input.append(str(round(value,2)))
			# ajout des alertes aux inputs
			self.liste_input.append(str(self.alerte_aeration))
			self.liste_input.append(str(self.alerte_intrusion))
			self.liste_input.append(str(self.alerte_incendie))
			
			# capture de l'image et ajout de ses données aux inputs			
			self.data_img_byte_array = self.capture_img()
			self.liste_input.append(self.data_img_byte_array)

			# re-set des valeur par défaut 
			self.alerte_aeration = False
			self.alerte_intrusion = False
			self.alerte_incendie = False


			#print(self.liste_input)
			...
			#	f.write(";".join(self.liste_input)+"\n")

			##### Ecriture dans les Topics MQTT
			self.client.loop_start()
			#self.client.publish(f'rpi_{str(self.client_id)}/time', str(self.liste_input[0]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/temperature', str(self.liste_input[1]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/humidity', str(self.liste_input[2]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/noise', str(self.liste_input[3]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/isPeopleDetected', str(self.liste_input[4]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/TVoC', str(self.liste_input[5]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/CO2eq', str(self.liste_input[6]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/alerte_aeration', str(self.liste_input[7]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/alerte_intrusion', str(self.liste_input[8]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/alerte_incendie', str(self.liste_input[9]), qos=0)
			self.client.publish(f'rpi_{str(self.client_id)}/byte_array_img', self.liste_input[10], qos=0)
	...
	def capture_img(self) : 
		...
		with open("./image.jpg", 'rb') as img :
			...
			img_byte_array = bytearray(img_content)
